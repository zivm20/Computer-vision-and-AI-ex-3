...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_center = x-running_mean
        sqrtV = np.sqrt(running_var+eps)
...
def batchnorm_backward(dout, cache):
    ...
    #                        -> dout/dxHatGamma = 1 -> dxHatGamma = dout*1
    dbeta = np.sum(dout,axis=0)
    dxHatGamma = dout*1

    #xHatGamma = xHat*gamma -> dxHatGamma/dgamma = xHat -> dgamma[i] = sum(xHat[:,i]*dxHatGamma[:,i])
    #                       -> dxHatGamma/dxHat = gamma -> dxHat[:,i] = gamma[i]*dxHatGamma[:,i]
    dgamma = np.sum(xHat*dxHatGamma,axis=0)
    dxHat = dxHatGamma*gamma

    #xHat = x_center/sqrtV -> dxHat/dx_center = 1/sqrtV -> dx_center[:,i] = (1/sqrtV[i])*dxHat[:,i]
    #                      -> dxHat/dsqrtV = -dx_center/(sqrtV^2) -> dsqrtV[i] = sum(-dx_center[:,i]/(sqrtV[i]^2)*dxHat[:,i])
    dx_center = dxHat*(1/sqrtV)
    dsqrtV = np.sum((-x_center/(sqrtV**2))*dxHat,axis=0)

    #sqrtV = sqrt(v+eps) -> dsqrtV/dv = 1/(2*sqrt(v+eps)) -> dv = dsqrtV/(2*sqrt(v+eps))
    dv = dsqrtV/(2*sqrtV)

    #v[i] = sum((x_center[:,i]**2))/N -> dv/dx_center = 2*x_center/N -> dx_center[:,i] = (dv[i])*(2*x_center[:,i]/N)
    dx_center += 2*x_center*dv/N


    #x_center = x-mean -> dx_center/dx = 1 -> dx = dx_center*1
    #                  -> dx_center/dmean = 1 -> dmean[i] = sum(-1*dx_center[:,i])
    dx = dx_center*1
    dmean = np.sum(-1*dx_center,axis=0)

    #mean[i] = sum(x[:,i])/N -> dmean/dx = 1/N -> dx[:,i] += dmean[i]*1/N
    dx = dx + dmean/N 
